...
pub fn decode(data: &[u8], width: u32, height: u32) -> Result<Heightmap, io::Error> {
	let pixel_count = width as usize * height as usize;
	let stream = decompress(data, pixel_count)?;
	...
	Ok(decode_prediction(predicted, width, height))
}

pub fn decompress(data: &[u8], pixel_count: usize) -> Result<Vec<u8>, io::Error> {
